...
# ======================================================
st.markdown("---")
st.subheader("📊 Key Factors Influencing Cancellation")



try:
    # Get final estimator and feature names from the pipeline
    clf = model.named_steps['model']            # XGBClassifier
    preprocess = model.named_steps['preprocess']  # ColumnTransformer

    # Importances from XGBClassifier
    if hasattr(clf, "feature_importances_"):
        ...
        sns.barplot(x='Importance', hue='Feature', data=importance_df, legend=False, palette='Blues_d', ax=ax)
        ...
        st.info("Feature importances are not available on this model.")
except Exception as e:
...
try:
    # Split pipeline: preprocess -> model (XGBClassifier)
    preprocess = model.named_steps['preprocess']
    clf = model.named_steps['model']

    # Transform current input row(s) to encoded numeric matrix
    X_enc = preprocess.transform(df_input)
    # If output is sparse, convert to dense for plotting
    X_enc_dense = X_enc.toarray() if hasattr(X_enc, "toarray") else X_enc

    # Feature names after ColumnTransformer (sklearn >= 1.0)
    try:
        feature_names = preprocess.get_feature_names_out()
    except Exception:
        feature_names = [f"f{i}" for i in range(X_enc_dense.shape[1])]

    # TreeExplainer for XGBoost; optionally explain probability space
    explainer = shap.TreeExplainer(clf)  # or shap.TreeExplainer(clf, model_output='probability')
    shap_values = explainer.shap_values(X_enc_dense)

    # Global importance bar
    fig, ax = plt.subplots(figsize=(8, 4))
    shap.summary_plot(shap_values, X_enc_dense, feature_names=feature_names, plot_type="bar", show=False)
    st.pyplot(fig)

    # Optional: local explanation for current booking (first row)
    # (TreeExplainer returns arrays; summary of a single row shown below.)
    fig2, ax2 = plt.subplots(figsize=(8, 4))
    shap.summary_plot(shap_values[:1], X_enc_dense[:1], feature_names=feature_names, show=False)
    st.pyplot(fig2)
